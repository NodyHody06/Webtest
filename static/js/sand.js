...
function draw() {
  ...
  // Draw the sand with glow effect
  for (let i = 0; i < cols; i++) {
    for (let j = 0; j < rows; j++) {
      if (grid[i][j] > 0) {
        let x = i * w;
        let y = j * w;
        let colorValue = color(grid[i][j], 255, 255);

        // Apply shadow blur for glow effect
        drawingContext.shadowBlur = 10;
        drawingContext.shadowColor = colorValue;
